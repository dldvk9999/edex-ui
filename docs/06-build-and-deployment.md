...
That fix alone turned out not to be sufficient, though: a `workflow_dispatch` run (guaranteed no event race, `PUBLISH_POLICY: always`) still landed 0 assets on `v2.3.1`. Every job stayed green and produced real installers (confirmed via the run's own `actions/upload-artifact` output), so the build side was never the problem - only the "attach to the already-published, non-draft `v2.3.1` release" step. This matches a long-standing, still-open class of `electron-builder` bug (`electron-userland/electron-builder#2393`, `#8179`, and others going back to 2017) where a persistent `releaseType`/`draft` config in `package.json` doesn't reliably reach the actual publish decision - the publisher ends up looking for a `draft` release, doesn't find one (since ours is already public), logs `existing type not compatible with publishing type ... existingType=release publishingType=draft`, and skips every single asset - again with a `0` exit code, invisible from CI status. The fix (also applied here): pass `-c.publish.draft=false -c.publish.prerelease=false -c.publish.releaseType=release` as explicit CLI overrides on every build/publish invocation, in addition to (not instead of) the `package.json` config - CLI-level publish config isn't subject to the same internal-state bug that config-file-only values apparently are.
