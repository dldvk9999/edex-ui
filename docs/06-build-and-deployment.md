...
That fix alone turned out not to be sufficient, though: a `workflow_dispatch` run (guaranteed no event race, `PUBLISH_POLICY: always`) still landed 0 assets on `v2.3.1`. Every job stayed green and produced real installers (confirmed via the run's own `actions/upload-artifact` output), so the build side was never the problem - only the "attach to the already-published, non-draft `v2.3.1` release" step. This matches a long-standing, still-open class of `electron-builder` bug (`electron-userland/electron-builder#2393`, `#8179`, and others going back to 2017) where a persistent `releaseType`/`draft` config in `package.json` doesn't reliably reach the actual publish decision - the publisher ends up looking for a `draft` release, doesn't find one (since ours is already public), logs `existing type not compatible with publishing type ... existingType=release publishingType=draft`, and skips every single asset - again with a `0` exit code, invisible from CI status.

A first attempt at fixing that (passing `-c.publish.draft=false -c.publish.prerelease=false -c.publish.releaseType=release` as CLI overrides on every build/publish invocation, alongside the `package.json` config) turned out to be actively wrong, not just insufficient - it made every job fail outright. Two distinct problems, found by actually reading the job logs (the GitHub API's workflow-run-logs endpoint redirects to Azure Blob Storage, which this project's own CI sandbox couldn't reach - screenshots from the GitHub mobile app were what actually surfaced these):

1. **`draft`/`prerelease` aren't valid `GithubOptions` fields in `electron-builder@23.6.0`** - only `releaseType` is. Passing them made the merged publish config fail AJV schema validation (`configuration.publish should be one of these: object { provider, ..., releaseType?, ... }` - note no `draft`/`prerelease` in that list) before the build even started, on every platform. This is what "the deprecated `--prerelease`/`--draft` flags" in older `electron-builder` issue threads actually means - they were removed for the `github` provider by this version, not just discouraged.
2. **`npm run <script> -- <extra args>` mangled the dot-notation CLI flags, but only on the jobs going through that indirection** (Windows' `npm run build-windows --`, macOS' `npm run build-darwin --`) - not on the Linux jobs, which already called `electron-builder` directly. Reproduced locally: `yargs` itself parses `-c.publish.releaseType=release` correctly in isolation (merges cleanly into a nested `config.publish` object) - something in npm's own script-argument-forwarding breaks that specifically, ending up passing a stray literal string (the tail end of one of the flags) as `-c`'s value, which `electron-builder` then tries to open as a config *file* path and fails with `ENOENT`.

Fix for both: dropped `draft`/`prerelease` from the CLI overrides entirely (`releaseType` alone is sufficient and valid), and switched the Windows/macOS jobs from `npm run <script> -- <args>` to invoking `electron-builder` directly (matching the Linux jobs, and adding `shell: bash` to the Windows step to match its own sibling steps, replacing the implicit default of PowerShell) - eliminating the argument-mangling path rather than working around it.
