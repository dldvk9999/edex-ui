...
4. ~~**Accessibility — remaining gaps**~~ — ✅ done, see below.
...
All low/low-medium/medium-high-effort items are now done (see 10.0 above). What's left is the High-effort tier: the theme editor GUI (self-contained, no architecture change), then the program launcher, the plugin system, and split panes - each a bigger, more cross-cutting undertaking than the last.
